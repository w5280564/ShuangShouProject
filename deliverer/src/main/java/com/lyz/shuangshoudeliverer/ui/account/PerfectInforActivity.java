...
import com.lyz.shuangshoudeliverer.ui.activity.HomeActivity;
import com.lyz.shuangshoudeliverer.ui.base.BaseActivity;
import com.lyz.shuangshoudeliverer.ui.bean.UserAccountBean;
import com.lyz.shuangshoudeliverer.utils.ShareSaveUtils;
...
public class PerfectInforActivity extends BaseActivity {
    ...
    @BindView(R.id.tvPerfectIDCard)
    EditText tvPerfectIDCard;
    @BindView(R.id.tvPerfectCompanyName)
    EditText tvPerfectCompanyName;
    private Uri imguri;
...
//                三星手机需要判断权限，让用户手动打开权限
            ...
            case R.id.btnPerfectFinish:
                ...
                if (TextUtils.isEmpty(tvPerfectIDCard.getText().toString().trim())) {
                    ToastUtils.getInstance().showToast("请输入身份证号");
                    return;
                }
                if (TextUtils.isEmpty(tvPerfectCompanyName.getText().toString().trim())) {
                    ToastUtils.getInstance().showToast("请输入商家名称");
                    return;
                }
    ...
    private void register() {
        RetrofitHelper.getNetworkService()
                .register(mUserName, mPsw, "1", tvPerfectIDCard.getText().toString().trim(), tvPerfectCompanyName.getText().toString().trim())
            ...
            switch (requestCode) {
                ...
                case GET_IMG_TAKE_PHOTO: //拍照
                    ...
                    try {
                        ...
                        file = new File(Environment.getExternalStorageDirectory(), "ShuangShou" + StaticaAdaptive.getStringRandom(6));
                        FileOutputStream out = new FileOutputStream(file);
                        myBitmap.compress(Bitmap.CompressFormat.PNG, 100, out);
                        out.flush();
                        out.close();
